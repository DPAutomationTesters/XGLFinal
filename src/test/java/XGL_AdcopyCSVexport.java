...
public class XGL_AdcopyCSVexport extends BaseClass
{
    @Test(priority = 1,enabled = true)
    public void OrderExportCSV()
    {
        AdCopy cp= new AdCopy(driver);
        cp.menuOrders.click();
        cp.menuAdcopy.click();
        SoftAssert sa=new SoftAssert();
        sa.assertTrue(cp.breadcrumvalue.getText().equalsIgnoreCase("Ad Copy"));
        //Download file
        ...
        if(crs.getrecordcountoflastpage()>0)
        {
            ...
            String expected= "AdCopy_"+ DateUtility.getDateinformat()+"_"+currentTime+".csv";
